        ...
        <div>
            ...
            <select wire:model.debounce="organization_id" id="orgs" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                @foreach($organizations as $organziation)
                    <option value="{{ $organziation->id }}">{{ $organziation->name ?? '' }}</option>
                @endforeach
            </select>
        </div>

        @if($designation_id)
            <x-button wire:click.debounce="updateDesignationHandeler" type="button" >Update</x-button>
        @else 
            <x-button wire:click.debounce="createDesignationHandeler" type="button" >Add</x-button>
        @endif
    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="flex-1 min-w-0">
                            <p class="text-md md:text-xl font-medium text-gray-900 truncate dark:text-white">
                                ...
                                <span class="ml-2 whitespace-nowrap rounded-full bg-purple-100 px-2.5 py-0.5 text-sm text-purple-700">
                                    {{ $designation->organization->name ?? 'None' }}
                                </span>
                            </p>
                        </div>
                        <div class="group-hover:block hidden">
                            <button wire:click.debounce="editHandeler({{ $designation->id }})">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                </svg>
                            </button>

                            <button wire:click.debounce="deleteHandeler({{ $designation->id }})" >
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
