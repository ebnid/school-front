<div>
   @if($is_edit_mode_on)
                ...
                <div class="p-7 max-w-2xl">
                    ...
                    <div class="grid grid-cols-1 gap-5">
                        ...
                        <div>
                            ...
                            <x-input wire:model.debounce="employee.subject" id="subject" class="block h-8 mt-1 w-full" type="text" />
                        </div>

                        <div>
                            <x-label for="subject_code" value="{{ __('Subject Code') }}" />
                            <x-input wire:model.debounce="employee.subject_code" id="subject_code" class="block h-8 mt-1 w-full" type="number" />
                        </div>

                        <div>
                            <x-label for="examinner_code" value="{{ __('Examinner Code') }}" />
                            <x-input wire:model.debounce="employee.examinner_code" id="examinner_code" class="block h-8 mt-1 w-full" type="number" />
                        </div>

                        <div>
                            <x-label for="training" value="{{ __('Training') }}" />
                            <x-input wire:model.debounce="employee.training" id="training" class="block h-8 mt-1 w-full" type="text" />
                        </div>

                        <div>
                            <x-label for="term" value="{{ __('Terms') }}" />
                            <x-input wire:model.debounce="employee.term" id="term" class="block h-8 mt-1 w-full" type="text" />
                        </div>


                        <div>
                            <label for="bio" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Bio</label>
                            <textarea wire:model.debounce="employee.bio" id="bio" rows="4" class="block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
                        </div>

                        <div>
                            <x-label for="employee_type" value="{{ __('Employee Type') }}" />
                            <x-ui.select wire:model.debounce="employee.employee_type" id="employee_type" class="block h-10 text-md mt-1 w-full">
                        ...
                        <div class="block">
                            <label for="is_published" class="flex items-center">
                                <x-checkbox wire:model.debounce="employee.is_published" id="is_published" />
                                <span class="ml-2 text-sm text-gray-600">{{ __('Published') }}</span>
                            </label>
                        </div>

                        <div class="flex items-center justify-end mt-4">
                            <x-button wire:click.debounce="saveEmployee" class="ml-4">
                                {{ __('Save') }}
                            </x-button>
                        </div>
        ...
        <!-- Loader -->
        <x-ui.loading-spinner wire:loading.flex wire:target="saveEmployee, image, cancelEditMode, removeTempProfile" />
   @endif
</div>