            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr class="group bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $withdraw->employee->user->name ?? '' }}
                        </th>
                        <td class="px-6 py-4">
                            {{ number_format($withdraw->amount, 2) }}
                        </td>
                        <td class="px-6 py-4">
                            {{ \Illuminate\Support\Str::limit($withdraw->message ?? '', 100) }}
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="group-hover:block hidden">
                                @if($withdraw->status === 'pending')
                                    <button wire:click.debounce="editHandeler({{ $withdraw->id }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                        </svg>
                                    </button>

                                    <button wire:click.debounce="deleteHandeler({{ $withdraw->id }})" >
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
@push('modals')
    <livewire:edit-withdraw-request />
@endpush